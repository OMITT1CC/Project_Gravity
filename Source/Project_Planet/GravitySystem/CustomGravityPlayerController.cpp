...
void ACustomGravityPlayerController::UpdateRotation(float DeltaTime)
{
	...
	else
	{
		if (GetPawn())
		{
			GravityDirection = GetPawn()->GetActorUpVector() * -1.f;
		}
	}
...
FRotator ACustomGravityPlayerController::GetGravityRelativeRotation(FRotator Rotation, FVector GravityDirection)
{
...
FRotator ACustomGravityPlayerController::GetGravityWorldRotation(FRotator Rotation, FVector GravityDirection)
{
	if (!GravityDirection.Equals(FVector::DownVector))
	{
